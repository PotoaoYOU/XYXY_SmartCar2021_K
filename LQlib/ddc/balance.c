...
void timer0_int(void) interrupt 1 //平衡车占用
{
    ...
    if (Servo_Integration < -900)
        Servo_Integration = -900;
    else if (Servo_Integration > 900)
        Servo_Integration = 900;
...
*/
    ...
    if (PWMServo < (Servo_Center - Servo_Delta))
        PWMServo = Servo_Center - Servo_Delta; //===舵机角度限制
    else if (PWMServo > (Servo_Center + Servo_Delta))
        PWMServo = Servo_Center + Servo_Delta; //===舵机角度限制
...
void OLED_Show(void)
{
    ...
    //=============第三行===============================//
    if (Flag_Stop)
        sprintf(txt, "Enc:%04d    Stop ", Encoder);
    else
        sprintf(txt, "Enc:%04d    Run ", Encoder);
...
int SBB_Get_Servo_Turn(int servo_input)
{
    ...
    {
        if (Flag_Right)
            LeftRightFlag = -120; //===向右转
        else if (Flag_Left)
            LeftRightFlag = 120; //===向左转
        else
            LeftRightFlag = 0;
    }                                                    /*
	else // 电感循迹
